...
for item in oracle_utxos:
    if item.output.datum and not item.output.script:
        ...
        oracle_datum_obj = PublishParams(
            oracle_datum.value[0],
            oracle_datum.value[1],
            oracle_datum.value[2]
        )
if not oracle_utxo:
...
for item in script_utxos:
    if item.output.script:
        ...
        if str(datum_obj.beneficiary.hex()) == str(beneficiary_vkey.hash()):
            beneficiary = Address(VerificationKeyHash.from_primitive(datum_obj.beneficiary), network=network)
            fee_address = Address(VerificationKeyHash.from_primitive(datum_obj.fee_address), network=network)
            """
...
for utxo_to_spend in claimable_utxos:
    ...
    builder.add_output(TransactionOutput.from_primitive([utxo_to_spend['fee_address'], utxo_to_spend['fee']]))
...
signed_tx = builder.build_and_sign([beneficiary_skey, collateral_skey], change_address=beneficiary_address)
