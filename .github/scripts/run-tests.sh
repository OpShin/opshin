...
for i in $(find examples/smart_contracts -type f -name "*.py" -not \( -name "broken*" -o -name "extract*" -o -name "wrapped_token*" -o -name "simple_script*" -o -name "parameterized*" -o -name "lp_minting_policy*" \)); do
...
uv run coverage run -a -m opshin compile "examples/smart_contracts/lp_minting_policy.py" --recursion-limit 4000 --parameters 1 > /dev/null || exit
