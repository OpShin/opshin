...
def compile_contract(
    ...
    cmd = [
        "poetry", "run", "opshin", "build", 
        purpose, contract_path, 
        f"-{optimization}", 
        "-o", output_dir
    ]
...
def generate_baseline(output_file: str, work_dir: Optional[str] = None, config_file: Optional[str] = None) -> None:
...
def compare_with_baseline(baseline_file: str, work_dir: Optional[str] = None, config_file: Optional[str] = None) -> bool:
    """
    ...
    for contract_config in config["contracts"]:
        ...
        for opt_level in config["optimization_levels"]:
            ...
            print(f"  {opt_level}: {baseline_size:,} → {current_size:,} bytes "
    ...
    if has_changes:
        ...
        print("Please review the changes and consider optimization if sizes increased significantly.")
    else:
...
def main():
    ...
    parser.add_argument(
        "--config-file",
        help="Configuration file (defaults to binary_size_config.yaml)"
    )
    ...
    if args.action == "generate":
        ...
        has_changes = compare_with_baseline(args.baseline_file, args.work_dir, args.config_file)
