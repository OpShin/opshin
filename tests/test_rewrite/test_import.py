...
"""
            ...
            result = eval_uplc_value(source_code, 3)
            assert result == 14, "Expected function_a(3) + variable_a (4 + 10 = 14)"
...
def test_selective_import_valid_2():
    """Test that selective imports work correctly"""
    # Create a test module content in memory
    test_module_code = """
def function_a(x: int) -> int:
...
def helper_function_b(x: int) -> int:
    return x * 2

def function_b(x: int) -> int:
    return helper_function_b(x) * 2
...
"""
    ...
    try:
        ...
        source_code = f"""
from {module_name} import function_b, variable_a

def validator(x: int) -> int:
    return function_b(x) + variable_a
"""

        # This should compile successfully
        result = eval_uplc_value(source_code, 3)
        assert result == 16, "Expected function_b(3) + variable_a (6 + 10 = 16)"
        source_code = f"""
from {module_name} import function_b
def validator(x: int) -> int:
    return function_b(x) + helper_function_b(x)  # helper_function_b not imported
"""

        # This should fail to compile
        with pytest.raises(CompilerError) as e:
            builder._compile(source_code)
        assert "helper_function_b" in str(
            e
        ), "Should fail because helper_function_b was not imported"
