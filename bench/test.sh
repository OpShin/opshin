...
then
    echo "\033[1mcbor-diag\033[0m missing from system but required. Install via `cargo install cbor-diag-cli`."
    exit 1
fi

if ! command -v aiken &> /dev/null
then
    echo "\033[1maiken\033[0m missing from system but required. Install via `cargo install aiken`."
    exit 1
fi
